...
while vid.CurrentTime < skipTime + 0.75
    ...
    fishCountPerFrame(end+1) = size(objCentroid, 1);
...
plotFishCountPerTime(frameTimes, fishCountPerFrame);
...
function plotFishCountPerTime(times, counts)
    figure;
    bar(times, counts);
    xlabel('Video Time (s)');
    ylabel('Number of Fish Detected');
    title('Fish Count per Frame');
    grid on;
end
